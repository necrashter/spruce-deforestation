...
import com.spruce.game.world.geom.CharHitBox;
...
public class GameEntity implements Damageable, Spatial {
    ...
    public void die() {
        dead = true;
        world.playSound(SpruceGame.assets.death[MathUtils.random.nextInt(SpruceGame.assets.death.length)], hitBox.position);
    }
    ...
    public boolean takeDamage(float amount, DamageAgent agent, DamageSource source) {
        ...
        if (source == DamageSource.Axe) {
            world.playSound(
                    SpruceGame.assets.stabs[MathUtils.random.nextInt(SpruceGame.assets.stabs.length)],
                    hitBox.position
            );
        }
